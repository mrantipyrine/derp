...
-- Compatibility helpers used by older combat scripts.
---@nodiscard
---@param target CBaseEntity
---@param dmg integer
---@return integer
function utils.handlePhalanx(target, dmg)
    if dmg > 0 then
        dmg = math.max(0, dmg - target:getMod(xi.mod.PHALANX))
    end
...
---@param dmg integer
---@return integer
function utils.handleStoneskin(target, dmg)
    return utils.stoneskin(target, dmg)
end
...
---@param dmg integer
---@return integer
function utils.handleOneForAll(target, dmg)
    return utils.oneforall(target, dmg)
end
