...
nr.conditionCache = nr.conditionCache or { zoneFamilies = {} }
...
local ambientFamilyPatterns =
{
    rabbit     = { 'rabbit', 'hare', 'rarab' },
...
local function normalizeAmbientName(name)
...
local function getZoneFamilyCounterVar(zoneId, family)
    return string.format('DW_NR_Z%d_%s', zoneId, string.upper(family))
end

local function getNamedRareCounterVar(key)
    return 'DW_NR_KILL_' .. string.upper(normalizeRareKey(key) or '')
end

local function getFamilyKillCount(zoneId, family)
    return GetServerVariable(getZoneFamilyCounterVar(zoneId, family)) or 0
end

local function addFamilyKillCount(zoneId, family, amount)
    local varName = getZoneFamilyCounterVar(zoneId, family)
    local current = GetServerVariable(varName) or 0
    local updated = math.max(0, current + (amount or 1))
    SetServerVariable(varName, updated)
    return updated
end

local function spendFamilyKillCount(zoneId, family, amount)
    local varName = getZoneFamilyCounterVar(zoneId, family)
    local current = GetServerVariable(varName) or 0
    local updated = math.max(0, current - (amount or 0))
    SetServerVariable(varName, updated)
    return updated
end

local function getNamedRareKillCount(key)
    return GetServerVariable(getNamedRareCounterVar(key)) or 0
end

local function addNamedRareKillCount(key, amount)
    local varName = getNamedRareCounterVar(key)
    local current = GetServerVariable(varName) or 0
    local updated = math.max(0, current + (amount or 1))
    SetServerVariable(varName, updated)
    return updated
...
local function spendNamedRareKillCount(key, amount)
    local varName = getNamedRareCounterVar(key)
    local current = GetServerVariable(varName) or 0
    local updated = math.max(0, current - (amount or 0))
    SetServerVariable(varName, updated)
...
local function matchesAmbientFamily(mobName, family)
    local patterns = ambientFamilyPatterns[family]
    if not patterns then
        return false
    end

    local normalized = normalizeAmbientName(mobName)
...
local function rebuildConditionCache()
    local cache = { zoneFamilies = {} }

    for key, config in pairs(nr.db or {}) do
        local conditions = config.conditions
        if conditions and conditions.killCount and conditions.killCount.family then
            local family = conditions.killCount.family
            local zones = conditions.killCount.zones or config.zones or {}

            for _, zoneEntry in ipairs(zones) do
                local zoneId = unwrapZoneId(zoneEntry)
                if zoneId then
                    cache.zoneFamilies[zoneId] = cache.zoneFamilies[zoneId] or {}
                    cache.zoneFamilies[zoneId][family] = true
                end
            end
        end
    end

    nr.conditionCache = cache
end

local function isNightTime()
    local totd = VanadielTOTD()
    return totd == xi.time.NIGHT or totd == xi.time.MIDNIGHT
end
...
local function zoneMatchesCondition(zoneId, conditionZones)
    if type(conditionZones) ~= 'table' or #conditionZones == 0 then
        return true
    end

    for _, zoneEntry in ipairs(conditionZones) do
        if unwrapZoneId(zoneEntry) == zoneId then
            return true
        end
    end
...
local function conditionsMet(key, config, zone)
    local conditions = config.conditions
    ...
    if conditions.killCount then
        local killCount = conditions.killCount
        local family = killCount.family
        local threshold = killCount.required or killCount.count or 0
        local counterZoneId = killCount.zoneId or zoneId
        if not zoneMatchesCondition(zoneId, killCount.zones) then
            return false
        end
        if not family or getFamilyKillCount(counterZoneId, family) < threshold then
            return false
        end
    end

    if conditions.namedRareKills then
        for depKey, required in pairs(conditions.namedRareKills) do
            if getNamedRareKillCount(depKey) < (required or 1) then
                return false
            end
        end
    end

    if conditions.weatherAny and not listContains(conditions.weatherAny, zone:getWeather()) then
        return false
    end

    if conditions.dayAny and not listContains(conditions.dayAny, VanadielDayOfTheWeek()) then
        return false
    end

    if conditions.nightOnly and not isNightTime() then
        return false
    end

    if conditions.randomRoll and math.random(1000) > conditions.randomRoll then
        return false
    end
...
local function consumeConditions(key, config, zoneId)
    local conditions = config.conditions
    if not conditions then
        return
    end

    if conditions.killCount and conditions.killCount.consume ~= false then
        local killCount = conditions.killCount
        local family = killCount.family
        local threshold = killCount.required or killCount.count or 0
        local counterZoneId = killCount.zoneId or zoneId
        if family and threshold > 0 then
            spendFamilyKillCount(counterZoneId, family, threshold)
        end
    end

    if conditions.namedRareKills and conditions.consumeNamedRareKills ~= false then
        for depKey, required in pairs(conditions.namedRareKills) do
            spendNamedRareKillCount(depKey, required or 1)
        end
    end
end

local function resolveAmbientTrackedFamily(zoneId, mobName)
    local zoneFamilies = nr.conditionCache.zoneFamilies[zoneId]
    if not zoneFamilies then
        return nil
    end

    for family, _ in pairs(zoneFamilies) do
        if matchesAmbientFamily(mobName, family) then
            return family
        end
    end
...
local function recordTrackedFamilyKill(zoneId, family, killer, amount)
    if not zoneId or not family then
        return nil
    end

    local player = xi.dynamicWorld.reputation.resolvePlayer(killer)
    if not player then
        return nil
    end

    return addFamilyKillCount(zoneId, family, amount or 1)
end

nr.recordTrackedFamilyKill = recordTrackedFamilyKill

nr.recordNamedRareProgress = function(key, config, zoneId, killer)
    addNamedRareKillCount(key, 1)

    local family = config and (config.family or getFamilyForKey(config.name) or getFamilyForKey(key))
    if family and zoneId then
        recordTrackedFamilyKill(zoneId, family, killer, 1)
    end
end

nr.bindAmbientListeners = function(zone)
    local zoneId = zone:getID()
    if not nr.conditionCache.zoneFamilies[zoneId] then
        return
    end
    ...
    for _, mob in pairs(mobs) do
        if mob and mob:getLocalVar('DW_NR_AMBIENT_WATCH') == 0 and mob:getLocalVar('DW_TIER') == 0 and mob:getLocalVar('DW_NR_TIER') == 0 then
            local family = resolveAmbientTrackedFamily(zoneId, mob:getName())
            if family then
                mob:setLocalVar('DW_NR_AMBIENT_WATCH', 1)
                mob:addListener('DEATH', 'DW_NR_AMBIENT_DEATH', function(deadMob, killer)
                    recordTrackedFamilyKill(zoneId, family, killer, 1)
                end)
            end
        end
...
nr.db = {
    ...
    cottontail_tommy =
    {
        ...
        conditions  =
        {
            killCount = { family = 'rabbit', required = 50 },
        },
        isAggro     = false,
        loot        =
        {
    ...
    mallow_mira =
    {
        ...
        conditions  =
        {
            killCount = { family = 'mandragora', required = 50 },
        },
        isAggro     = false,
        loot        =
        {
    ...
    wiggles_winston =
    {
        ...
        family      = 'worm',
        zones       = { xi.zone.NORTH_GUSTABERG, xi.zone.SOUTH_GUSTABERG },
        level       = { 1, 5 },
        spawnTimer  = 10 * 3600,
        spawnWindow = 15 * 60,
        spawnChance = 200,
        conditions  =
        {
            killCount = { family = 'worm', required = 35 },
            dayAny    = { xi.day.EARTHSDAY },
        },
        isAggro     = false,
        loot        =
        {
    ...
    scaly_sally =
    {
        ...
        conditions  =
        {
            killCount = { family = 'lizard', required = 50 },
        },
        isAggro     = true,
        loot        =
        {
    ...
    little_jim =
    {
        ...
        conditions  =
        {
            killCount = { family = 'goblin', required = 40 },
        },
        isAggro     = true,
        loot        =
        {
    ...
    just_jim =
    {
        ...
        conditions  =
        {
            namedRareKills = { little_jim = 1, big_jim = 1 },
            randomRoll     = 350,
        },
        isAggro     = true,
        loot        =
        {
            { itemId = 19402, rate = 10479  },    -- Jim Cleaver
            { itemId = 18972, rate = 350 },       -- Storm Knucks
            { itemId = 15796, rate = 200 },       -- Big Jim Cape
        },
        deathMsg    = 'Just Jim falls. The puzzle was the real joke.',
    },
    ...
    ["flittering_fiona"] = {
        ...
        spawnChance = 220,
        conditions = {
            killCount = { family = "bat", required = 35 },
            nightOnly = true,
        },
        isAggro = true,
        loot = {
    ...
    ["snapping_sicily"] = {
        ...
        spawnChance = 220,
        conditions = {
            killCount = { family = "pugil", required = 40 },
            weatherAny = { xi.weather.RAIN, xi.weather.SQUALL },
        },
        isAggro = true,
        loot = {
    ...
    ["bone_dandy_boris"] = {
        ...
        spawnChance = 180,
        conditions = {
            killCount = { family = "skeleton", required = 30 },
            nightOnly = true,
            dayAny = { xi.day.DARKSDAY },
        },
        isAggro = true,
        loot = {
    ...
    ["karakul_karl"] = {
        ...
        zones = { xi.zone.LA_THEINE_PLATEAU, xi.zone.KONSCHTAT_HIGHLANDS },
        level = { 18, 24 },
        spawnTimer = 16 * 3600,
        spawnWindow = 4 * 3600,
        spawnChance = 80,
...
nr.trySpawn = function(key)
    ...
    if not conditionsMet(key, config, zone) then
        return false
    end
    ...
    local entityTable =
    {
        ...
        onMobDeath            = function(mob, player, optParams)
            ...
            nr.alive[key] = nil
            nr.recordNamedRareProgress(key, config, mob:getZoneID(), player)
    ...
    if mob then
        ...
        nr.alive[key] = mob
        consumeConditions(key, config, zoneId)
...
nr.forceSpawn = function(key, player)
    ...
    local entityTable =
    {
        ...
        onMobDeath            = function(mob, killer, optParams)
            SetServerVariable('DW_NR_' .. key, os.time())
            nr.alive[key] = nil
            nr.recordNamedRareProgress(key, config, mob:getZoneID(), killer)
            if config.deathMsg then
                local z = mob:getZone()
                if z then xi.dynamicWorld.announceZone(z, config.deathMsg) end
            end
