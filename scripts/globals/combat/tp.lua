...
xi.combat.tp.calculateTPGainOnPhysicalDamage = function(totalDamage, delay, actor, target)
    -- Compatibility for older callers that still pass (actor, target, damage, delay).
    if type(totalDamage) ~= 'number' and type(actor) == 'number' and type(target) == 'number' then
        totalDamage, delay, actor, target = actor, target, totalDamage, delay
    end

    if type(totalDamage) ~= 'number' then
