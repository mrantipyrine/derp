...
spellObject.onSpellCast = function(caster, target, spell)
    ...
    if mainJob == xi.job.RDM or subJob == xi.job.RDM then
        caster:addStatusEffect(xi.effect.ENFIRE, enstonePower, 3, duration, 0, 10, 1)
    end
    ...
    if mainJob == xi.job.RDM or mainJob == xi.job.BLM or mainJob == xi.job.WHM or
       subJob == xi.job.RDM or subJob == xi.job.BLM or subJob == xi.job.WHM then
        caster:addStatusEffect(xi.effect.BLAZE_SPIKES, stoneskinPower, 3, duration, 0, 10, 1)
    end
    ...
    if day == xi.day.FIRESDAY and mainJob == xi.job.BLM and math.random() <= 0.40 then
